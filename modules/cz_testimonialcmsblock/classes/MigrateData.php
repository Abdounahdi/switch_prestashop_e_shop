...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'cztestimonialcmsblockinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'cztestimonialcmsblockinfo_lang` il ON il.`id_cztestimonialcmsblockinfo` = i.`id_cztestimonialcmsblockinfo`'
        );
    ...
    /**
     * Import the old TestimonialCmsBlock data in the new structure
     *
     * @return bool
     */
    public function insertData()
    {
        ...
        $cztestimonialcmsblockinfo->text = reset($customTexts);
