...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'czoffercmsblockinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'czoffercmsblockinfo_lang` il ON il.`id_czoffercmsblockinfo` = i.`id_czoffercmsblockinfo`'
        );
    ...
    /**
     * Import the old OfferCmsBlock data in the new structure
     *
     * @return bool
     */
    public function insertData()
    {
        ...
        $czoffercmsblockinfo->text = reset($customTexts);
