...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'czheadercmsblockinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'czheadercmsblockinfo_lang` il ON il.`id_czheadercmsblockinfo` = i.`id_czheadercmsblockinfo`'
        );
    ...
    /**
     * Import the old HeaderCmsBlock data in the new structure
     *
     * @return bool
     */
    public function insertData()
    {
        ...
        $czheadercmsblockinfo->text = reset($customTexts);
